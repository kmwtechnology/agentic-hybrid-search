...
- `langchain_agent/` — Main application (all development happens here)
  ...
  - `relevancy_metrics.py` — Pure-function ranking metrics (`dcg`, `ndcg_at_k`, `mrr`, `recall_at_k`, `precision_at_k`, `compute_stage_metrics`, `confidence_from_scores`, `count_rank_changes`, `latency_cost_benefit`). Zero external deps so it ships unchanged on Cloud Run.
  ...
  - `ingest_esci_judgments.py` — ESCI per-query relevance judgments → OpenSearch `esci_judgments` index (~97k US queries / ~1.8M nested judgments). Used by the Pipeline Quality Summary for ground-truth NDCG/MRR/Recall/Precision.
  ...
  - `web/` — React 18 + TypeScript + Tailwind frontend
    ...
    - `web/src/components/ObservabilityPanel/PipelineSummaryCard.tsx` — Renders the Pipeline Quality Summary (BM25 vs Hybrid vs Reranked NDCG@10/MRR/Recall@20/Precision@10 + lift-per-100ms; falls back to confidence-proxy layout when no judgments are available)
    - `web/vite.config.ts` — Vite build configuration
  - `tests/` — pytest suites: `unit/`, `integration/`, `e2e/` with ~588 unit tests covering exceptions, auth middleware, origin auth, health routes, routing functions, pipeline nodes, vector store, embedding cache, retry utils, link verifier, reranker, conversations, and more
...
# Pre-commit hook (installed locally, not committed to repo)
# .git/hooks/pre-commit runs black + isort + flake8 on every staged .py file automatically.
# If it blocks a commit: cd langchain_agent && make format-fix, then re-stage and retry.
make ci-frontend              # frontend tests + eslint + tsc only
make ci-docker                # validate Dockerfile builds locally (no push)
...
npm run lint                  # eslint
npm run test                  # Vitest: 101 tests (stores, hooks, components)
...
- `PipelineSummaryEvent` — Per-stage retrieval quality summary (BM25 / Hybrid / Reranked NDCG@10, MRR, Recall@20, Precision@10 + per-stage latency and lift-per-100ms). Falls back to a confidence-proxy payload when no ESCI judgments exist for the query. Emitted once after `AgentCompleteEvent`.
...
- **State access**: `CustomAgentState` uses `total=False` with ~15 fields. Only `messages` is guaranteed to exist. Always use `state.get("field", default)`, never `state["field"]` for optional fields.
  ...
  The retriever runs the hybrid search and a BM25-only baseline in parallel (2-thread `ThreadPoolExecutor`; opensearch-py is sync but releases the GIL during I/O). `pre_rerank_documents` is the hybrid result before reranking, `bm25_documents` is the lexical baseline, and `judgments` is the per-query ESCI judgment lookup (or `None` when no row exists in `esci_judgments`). All three feed `PipelineSummaryEvent`.
...
## ESCI Judgments Ingestion

The `ingest_esci_judgments.py` script loads Amazon ESCI per-query relevance judgments into the `esci_judgments` OpenSearch index. These power the ground-truth NDCG/MRR/Recall/Precision numbers shown in the Pipeline Quality Summary card.

### Data Flow
1. **Load**: Reads `esci/shopping_queries_dataset/shopping_queries_dataset_examples.parquet` (~2.6M rows globally)
2. **Filter**: Selects US locale (`product_locale == "us"`) → ~1.8M judgment rows over ~97k unique queries
3. **Group**: Aggregates per-query nested judgments `[{product_id, label, relevance}, ...]`. ESCI labels map to graded relevance E=4.0, S=1.0, C=0.1, I=0.0 — used directly by `compute_stage_metrics`
4. **Index**: Bulk uploads to `esci_judgments` (one document per query, judgments stored as a nested field)

### Index Schema
- Document `_id` is the query string itself
- `query` (text + `query.keyword` for exact lookup), `query_id` (long), `n_judgments` (long)
- `judgments` (nested): `product_id` (keyword), `label` (keyword: E/S/C/I), `relevance` (float)

### CLI
```bash
PYTHONPATH=. python ingest_esci_judgments.py            # default: --reset, US locale
PYTHONPATH=. python ingest_esci_judgments.py --limit 1000  # smaller sample for dev
PYTHONPATH=. python ingest_esci_judgments.py --locale us   # locale filter
PYTHONPATH=. python ingest_esci_judgments.py --append      # add to existing index instead of recreating
PYTHONPATH=. python ingest_esci_judgments.py --stats       # show index statistics
```

### Lookup at query time
`OpenSearchVectorStore.lookup_judgments(query)` does an exact `term` match on `query.keyword`. If no judgment row exists, the Pipeline Quality Summary falls back to a confidence-proxy layout (reranker score distribution + rank-change count) instead of fabricating numbers — fuzzy matching was deliberately rejected to avoid false-positive judgment sets.
...
- **Unit tests** (~588 Python + 101 frontend): No external dependencies, all mocked. Python tests run in ~0.5s via `PYTHONPATH=. pytest tests/unit/`. Frontend tests (Vitest) run via `npm run test` from `langchain_agent/web/`.
- **CRITICAL — when modifying `tests/e2e/` or `tests/integration/`**: `make ci` runs `pytest --collect-only` against both directories specifically to catch import errors, signature errors, renamed exceptions, and library API drift (e.g., `websockets` v14 renamed `extra_headers` → `additional_headers` and moved the connect API from `websockets.client` to `websockets.asyncio.client`). Collection ≠ execution, but it catches the failure modes `tests/unit/` doesn't exercise. **Do not push e2e/integration changes without running `make ci`** — these tests only run live in CI against Cloud Run, and broken kwargs/imports will not surface any other way.
...
- `EMBEDDINGS_MODEL=models/text-embedding-005` — Embedding model (768-dim, set output_dimensionality=768)
